...
let userSettings = {
  ...
  target_lang: "id",
  tts_enabled: true,
  virtual_mic_enabled: false, // OFF by default for YouTube/media; turn ON for Zoom/Meet
  ducking_level: 0.2 // 20% volume during TTS playback
};
...
async function setupOffscreenDocument(path) {
  ...
  // Always close and recreate to ensure latest offscreen.js is loaded
...
function connectAndStart(tabId, tabStreamId, sendResponse) {
  ...
  const wsUrl = `ws://localhost:8000/ws/translate?user_id=${encodeURIComponent(userSettings.user_id)}`;
  ...
  try {
    ...
    socket.onopen = async () => {
      try {
        ...
        // Notify active tab to display UI overlay immediately
        if (activeTabId) {
          try {
            await chrome.scripting.insertCSS({ target: { tabId: activeTabId }, files: ["styles.css"] }).catch(() => {});
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === "translation_result") {
        if (activeTabId) {
          // Send subtitle to content script
...
function startOffscreenRecording(tabStreamId, sendResponse, tabId, hasRespondedObj) {
  chrome.runtime.sendMessage({ 
    ...
    chrome.tabs.sendMessage(tabId, { action: "TRANSLATION_STARTED", settings: userSettings }, () => {
      // ignore lastError for tabs.sendMessage in case content script isn't loaded
...
function stopTranslation() {
  ...
  // We no longer close the offscreen document here to prevent race conditions if the user rapidly restarts.
  // The media streams are already stopped by STOP_RECORDING, so the recording indicator will disappear.
